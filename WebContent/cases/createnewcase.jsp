...
$('#add_rcv_dtls').click(function(){
	...
	$.get('../TLKMultiRows?loadRcvRow='+RCVno , dataToSend,function(data, status){ 
		if (status=='success'){
...
function loadDistrict(seq){
	...
	if (destCity !== null && destCity=='BAS'){
		...
		$("#rcv_district_smartyNewRow_"+seq).prop("disabled", false);
	}else{
		$("#rcv_district_smartyNewRow_"+seq).removeAttr("required");
		$("#rcv_district_smartyNewRow_"+seq).prop("disabled", true);
...
function calcShipmentCost(seq){
	...
	if (senderCompany === undefined || senderCompany===null)
		return ;
	
	var dataToSend = {"destState":destCity, "originState" :originState, "rural":rural, "custName":custName, "senderCompany":senderCompany};
...
$('#sendercompany').on('change',function (){
	for (i=1; i<=rowNum; i++)
		calcShipmentCost (i);
});
</script>
