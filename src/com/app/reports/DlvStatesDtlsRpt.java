...
public class DlvStatesDtlsRpt extends CoreMgr{
	...
	public DlvStatesDtlsRpt(){
		 ...
		 */
		MainSql = "SELECT st_name_ar, COUNT(*) AS value_occurrence FROM p_casesmaster join `kbstate` on `st_code` =  `cm_pickup_state`"
				+ " where 1=0  GROUP BY cm_pickup_state";
	
		mainTable = "p_casesmaster";

		/*
		 * to define user grid views caption
		 */
		userDefinedCaption = "تفاصيل الشحنات لكل محافظة علي حسب أكتر طلب إرسال";
	...
	public void initialize(HashMap smartyStateMap) {
			...
			for (String value : search_paramval.get(parameter)) {
				if (!parameter.equals("filter") && (value != null) && (!value.equals(""))) {
					...
					if (parameter.equals("fromDate")) {
						fromExpDt = " cm_createddt >= DATE_FORMAT('" + value
								+ "', '%Y-%m-%d')";
						from = value;
						foundSearch = true;
					} else if (parameter.equals("toDate")) {
						toExpDt = " cm_createddt <= DATE_FORMAT('" + value
								+ "', '%Y-%m-%d')";
						to = value;
					} 
		...
		if (foundSearch) {
			...
			MainSql =  "SELECT st_name_ar, COUNT(*) AS value_occurrence FROM p_casesmaster join `kbstate` on `st_code` =  `cm_pickup_state`"
					 + "where" +whereClause+" GROUP BY cm_pickup_state";
