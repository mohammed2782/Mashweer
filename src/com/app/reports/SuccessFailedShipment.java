...
public class SuccessFailedShipment extends CoreMgr {
	public SuccessFailedShipment() {
		...
		MainSql = "select q_id ,sum((case when  q_code in ('dlv_stg__delivered') and q_status!='CLS' then 1 else 0 end))as " +
				"success, sum((case when  q_stage='cncl' and q_status!='CLS' then 1 else 0 end))as" +
				" failed from p_queue join p_cases on c_id = q_caseid and q_status !='CLS'  where 1=0 " +
	...
	public void initialize(HashMap smartyStateMap) {
		...
		if (foundSearch) {
			MainSql = "select sum((case when  q_code in ('dlv_stg__delivered') and q_status!='CLS' then 1 else 0 end))as success, " +
					"sum((case when   q_stage='cncl' and q_status!='CLS'  then 1 else 0 end))as failed " +
					"from p_queue join p_cases on c_id = q_caseid and q_status !='CLS'  where"
					+ fromExpDt + " and " + toExpDt  ;
