...
public class NewCases extends CoreMgr{
	...
	public NewCases () {
		...
		MainSql =  "select '' as sendercompany, '' as c_createddt , '' as assignagent, '' as createddtls, '' as c_cust_hp, '' as c_cust_name, '' as c_pickup_lat ,'' as c_pickup_longt , '' as usemap, "
	...
	public StringBuilder getRCVDetailsRow(int rcvSeq) {
		...
		String defaultRcvName = "", fragile = "" , rcvPhone="07", destState="BAS", rcvDistrict="", rural ="N" , rmk="", locDtls="", 
				 brinBackItmes="", receiptAmt="", qty="1", shipmentCost = "0" , receiptNo="" ,custName ="";
		...
		try {
			...
			if (rural.equalsIgnoreCase("Y"))
				ruralArea = true;
			shipmentCost = Double.toString(ut.calcShipmentChargesBasedOnDestCity(conn2,  destState, ruralArea, custName,0));
		} catch (Exception e1) {
		...
		sb.append("<div class='col-md-1 col-sm-1 col-xs-3 ' style='margin-left:2%;margin-right:2%;'><label class='control-label'>المدينه</label>"
				+ ""
				+ "<select class='form-control select2_single'  onchange='calcShipmentCost("+rcvSeq+");loadDistrict("+rcvSeq+");' id='rcv_city_"+userDefinedMultiNewRowExtension+"'" + 
		...
		if (fragile.equalsIgnoreCase("Y"))
			checked="checked";
		sb.append("<div class='form-group'><div class='col-md-1 col-sm-1 col-xs-3' style='margin-left:1%;margin-right:1%;'><label class='control-label'>قابل للكسر</label>"
						+ "</br>"
		...
		sb.append("<div class='col-md-1 col-sm-2 col-xs-3' "+stylediv+"><label class='control-label'>المستلم</label>"
				...
				+ " name='rcv_name_"+userDefinedMultiNewRowExtension+"' id ='rcv_name_"+userDefinedMultiNewRowExtension+"' size='7'  "
						+ " value='"+defaultRcvName+"'  /></div>");

		//Receiver Phone
		sb.append("<div class='col-md-1 col-sm-1 col-xs-3' style='margin-left:1%;margin-right:1%;'><label class='control-label'>هاتف</label>"
				+ ""
		...
		// send money
				sb.append("<div class='col-md-1 col-sm-2 col-xs-3' style='margin-left:2%;margin-right:2%;'><label class='control-label'>كود المرسل</label>"
						+ ""
						+ "<input type='text' class='form-control'  style='text-align:right; color: #424242;' "
						+ " name='c_specialsendercode_"+userDefinedMultiNewRowExtension+"' id ='c_specialsendercode_"+userDefinedMultiNewRowExtension+"' size='9'  "
								+ "  /></div>");
				
		//No of Pieces
		sb.append("<div class='col-md-1 col-sm-1 col-xs-3 ' style='margin-left:2%;margin-right:2%;'><label class='control-label'>عدد القطع</label>"
				+ ""
		...
		sb.append("<div class='col-md-1 col-sm-1 col-xs-3' style='margin-right:1%;margin-left:1%;'>"
				+ " <label class='control-label'>مندوب التوصيل</label>"
				+ "<select class='form-control select2_single'   id='c_assignedagent_"+userDefinedMultiNewRowExtension+"'" + 
				"  name='c_assignedagent_"+userDefinedMultiNewRowExtension+"'  >");
		sb.append("<option value='' selected></option> \n");
		...
		//shipment cost
		sb.append("<div class='col-md-1 col-sm-1 col-xs-3' style='margin-left:2%;margin-right:2%;'><label class='control-label'>مبلغ الشحن</label>"
						+ "<input type='number' min='0' readonly  class='form-control' value='"+shipmentCost+"' style='width:8em;'  size='10' name='c_shipment_cost_"+userDefinedMultiNewRowExtension+"' "
	...
	public String doInsert(HttpServletRequest rqs, boolean autocommit) { 
		...
		try{
			...
			for (Integer j : availableCases) {
				...
				// when the delivery agent is selected then save it
				...
				if(inputMap_ori.containsKey("rcv_district_"+userDefinedMultiNewRowExtension+"_"+j))
				...
				ci.setSpecialSenderCode(inputMap_ori.get("c_specialsendercode_"+userDefinedMultiNewRowExtension+"_"+j)[0]);
				...
				/*
				if (inputMap_ori.containsKey("c_bringitemsback_"+userDefinedMultiNewRowExtension+"_"+j)&&(inputMap_ori.get("c_bringitemsback_"+userDefinedMultiNewRowExtension+"_"+j)[0] !=null))
					ci.setBrinBackitems("Y"); // fragile
				else
					ci.setBrinBackitems("N");
				*/
				ruralArea = false;
				if (ci.getRural().equalsIgnoreCase("Y"))
					ruralArea = true;
				ci.setShipmentCharge(ut.calcShipmentChargesBasedOnDestCity(conn, ci.getState(),ruralArea,custid,caseMaster.getSenderCompanyId()));
				
				ci.setRemainingAmt(ci.getReceiptAmt()-ci.getShipmentCharge());//we use this to set the balance same as goods cost
				
				
				agentShareAmt = ut.calcAgentShipmentChargesShare(conn, caseMaster.getSenderCompanyId(), ci.getState(), ci.getDistrict() , ruralArea , ci.getAssignedDLVAgent());
				ci.setAgentShare(agentShareAmt);
				cases.add(ci);
			}
			try {pst.close();}catch(Exception e) {/*ignore*/}
			...
			if (pickUpAgent == null || pickUpAgent.equalsIgnoreCase("") || pickUpAgent.equalsIgnoreCase("0")) {
			...
			pst = conn.prepareStatement("insert into p_cases "
					...
					+ "	 c_agentshare	, c_rcv_district	, c_custid		 	, c_custhp			, c_pickupagent , "
					+ "  c_assignedagent, c_company_sender  , c_createddt	 	, c_specialsendercode)"
			+ " values  (?			    , ?			     	, ?		 			, ?,"
			+ "			 ?				, ?				    , ?			     	, ?		 			, ?,"
			+ "          ?				, ?				    , ?              	, ?		 			, ?,"
			+ "			 ?				, ?				 	, ?		 		 	, ?		 			, ?, "
			...
			for (int i =0; i<cases.size(); i++) {
					...
					pst.setString(19, pickUpAgent);
					pst.setString(20, ci.getAssignedDLVAgent());
					pst.setInt(21, caseMaster.getSenderCompanyId());
					pst.setString(22, inputMap_ori.get("c_createddt")[0]);
					pst.setString(23, ci.getSpecialSenderCode());
					pst.executeUpdate();
