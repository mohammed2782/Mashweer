...
public class NewCasesBarCode extends CoreMgr{
	...
	public StringBuilder getRCVDetailsRow(int rcvSeq, String rcpNo) throws Exception {
		...
		String defaultRcvName = "", fragile = "" , rcvPhone="07", destState="BGD", rcvDistrict="", rural ="N" , rmk="", locDtls="", brinBackItmes="", 
				receiptAmt="", qty="1", shipmentCost = "0" , receiptNo=rcpNo, custName = "" ;
		...
		try {
			...
			if (rural.equalsIgnoreCase("Y"))
				ruralArea = true;
			shipmentCost = Double.toString(ut.calcShipmentChargesBasedOnDestCity(conn1,  destState, ruralArea, custId,0));
		} catch (Exception e) {
			throw e;
		}finally {
		...
		if (custId>0) {
			...
			sb.append("<div class='col-md-1 col-sm-1 col-xs-3 ' style='margin-left:7%;margin-right:2%;'><label class='control-label'>الزبون</label>"
						+ "<select class='form-control select2_single' onchange='calcShipmentCost("+rcvSeq+")'  id='custid_"+userDefinedMultiNewRowExtension+"'" + 
		...
		*/
		...
		if (destState.equalsIgnoreCase("BGD")) {
			sb.append("<div class='col-md-2 col-sm-1 col-xs-3' style='margin-right:3%;margin-left:3%;'><label class='control-label'>مندوب التوصيل بغداد فقط</label>"
		...
		try{
			...
			for (Integer j : availableCases) {
				...
				if (ci.getRural().equalsIgnoreCase("Y"))
					ruralArea = true;
				ci.setShipmentCharge(ut.calcShipmentChargesBasedOnDestCity(conn, ci.getState(),ruralArea,ci.getCustId(),0));
				
				ci.setRemainingAmt(ci.getReceiptAmt()-ci.getShipmentCharge());//we use this to set the balance same as goods cost
				
				if (inputMap_ori.containsKey("c_company_sender_"+userDefinedMultiNewRowExtension+"_"+j)&&(inputMap_ori.get("c_company_sender_"+userDefinedMultiNewRowExtension+"_"+j)[0] !=null))
					ci.setSenderCompanyId(inputMap_ori.get("c_company_sender_"+userDefinedMultiNewRowExtension+"_"+j)[0]);
				else
					ci.setSenderCompanyId("0");
				
				
				agentShareAmt = ut.calcAgentShipmentChargesShare(conn, Integer.parseInt(ci.getSenderCompanyId()), ci.getState(), ci.getDistrict() , ruralArea , ci.getAssignedDLVAgent());
			...
			pst = conn.prepareStatement("insert into p_cases "
					...
					+ "  c_receiptfromsystem, c_createddt	 	)"
			+ " values  (?			    	, ?			     	 , ?		 		, ?					, ?				, "
			+ "			 ?				    , ?			    	 , ?		 		, ?					, ?				, "
			+ "			 ?				    , ?              	 , ?	 			, ?					, ?				, "
			+ "			 ?				 	, ?		 			 , ?	 			, ?					, ? 			, "
			+ "			'Y'				 	, (now()+INTERVAL 9 HOUR) 	 )",
