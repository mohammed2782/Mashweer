...
public class PickUpAgentBalance extends CoreMgr {
	...
	public PickUpAgentBalance() {
		records = 0;
		MainSql = "select c_shipmentpaidbysender, c_pickupagent, '' as selectedcases,'' as selectedcaseshidden, q_stage, q_step,'' as status ,'' as pmtCheckBox,  'شحنات سلمت وراجعه فقط' as title, c_custid,c_custreceiptnoori,'' as totamt,'' as pmtrmk, '' as pmtdate, "
				...
				+ " (case when (q_stage='dlv_stg' and q_step='delivered') then c_receiptamt else 0 end) as c_receiptamt,"
				+ " (case when (q_stage='dlv_stg' and q_step='delivered') then c_sendmoney else 0 end) as c_sendmoney,"
				+ " (case when ( (q_stage='dlv_stg' and q_step='delivered') "
				+ "				or ( q_stage='cncl' and q_status !='CLS' and c_shipmentpaidbysender='Y')"
				+ "			    ) "
				+ "				then c_shipment_cost else 0  end) as c_shipment_cost, "
				+ " (case when (q_stage='dlv_stg' and q_step='delivered')  then (c_receiptamt -  c_sendmoney - c_shipment_cost) else  (1*-c_shipment_cost)  end) as netamt "
				+ " from p_cases "
				...
				+ " where c_pickupagent ={pickupAgentAcct} and c_settled !='FULL'  and c_receiptfromsystem='N' "
				+ " and ( (q_stage='dlv_stg' and q_step='delivered')"
				+ " 		or ( q_stage='cncl' and q_status !='CLS' and c_shipmentpaidbysender='Y')) "
				+ "  and (c_branchcode='{userstorecode}' or '{superRank}'='Y') order by c_custreceiptnoori ";
		...
		userDefinedNewColsDefualtValues.put("totamt", new String [] {"%select "
				+ "(sum( (case when (q_stage='dlv_stg' and q_step='delivered') then c_receiptamt else 0 end)) - sum((c_shipment_cost)) - sum(c_sendmoney))  "
				+ " from p_cases "
				+ " join p_queue "
				+ "		on (c_id = q_caseid and q_status !='CLS'"
				+ "			 and ( "
				+ "					(q_stage='dlv_stg' and q_step='delivered')"
				+ "				  or (q_stage='cncl'  and c_shipmentpaidbysender='Y') "
				+ "				  )"
				+ "			) "
				+ " where c_pickupagent='{pickupAgentAcct}'  and c_settled !='FULL' and c_receiptfromsystem='N'  "});
	...
	public void initialize(HashMap smartyStateMap){
		...
		String pickUpAgent = replaceVarsinString(" {pickupAgentAcct} ", arrayGlobals).trim();
		...
		try {
			...
			pst = conn2.prepareStatement("select c_custreceiptnoori "
				...
				+ " join p_queue on (c_id= q_caseid and q_status !='CLS')"
				+ " left join kbstate on st_code = c_rcv_state"
				+ " where c_pickupagent =? and c_settled !='FULL' "
				+ " and (q_stage='dlv_stg' and q_step='delivered')  and c_receiptfromsystem='N' "
			...
			String sql = "select c_custreceiptnoori ,ifnull( (sum((case when c_shipmentpaidbysender ='Y' then 0 else c_receiptamt end)) - sum((c_shipment_cost)) - sum(c_sendmoney)),0)as totdue "
			...
			if (checkBoxPayment) {
				...
				userDefinedNewLookups.put("totamt", "!select ifnull( (sum( (case when c_shipmentpaidbysender ='Y' then 0 else c_receiptamt end)) - sum((c_shipment_cost)) - sum(c_sendmoney)),0)  from  p_cases "
						+ " join p_queue on (c_id = q_caseid and q_status !='CLS' and "
						+ "		( (q_stage='dlv_stg' and q_step='delivered') "
						+ "			or ( q_stage='cncl' and q_status !='CLS' and c_shipmentpaidbysender='Y')"
						+ "		) )"
						+ " where c_pickupagent='"+pickUpAgent+"'  and c_settled !='FULL' and c_receiptfromsystem='N'  and  (c_createddt >= '{fromdate}' "
	...
	public String doInsert(HttpServletRequest rqs, boolean autoCommit) {
		...
		String pickUpAgentId = replaceVarsinString(" {pickupAgentAcct} ", arrayGlobals).trim();
		...
		try {
			...
			String pickUpAgentIdFromRequest = inputMap_ori.get("c_pickupagent")[0];
			...
			if (! pickUpAgentIdFromRequest.equalsIgnoreCase(pickUpAgentId))
				throw new Exception ("Error, customer in form is ("+pickUpAgentIdFromRequest+") and global customer id is ("+pickUpAgentId+") are not the same");
			
			double calpmtAmt = 0.0;
			if (!checkBoxPayment) {
				String fromDate = inputMap_ori.get("fromdate")[0];
				String toDate = inputMap_ori.get("todate")[0];
				calpmtAmt = getTotaAmtToPayForCustomersUnderPickUpAgent (conn , pickUpAgentIdFromRequest , fromDate, toDate);
				//get list of cids
				cidList = getCid(conn , pickUpAgentId , fromDate, toDate);
			}else {
			...
			if (!cidList.isEmpty()) {//make sure u have cases
				ps = conn.prepareStatement( "INSERT INTO p_customer_payments_pickupagents "
						+ " 		(cppa_pickupagentid, cppa_amount_paid, cppa_paymentdt, cppa_createdby,cppa_rmk)"
						+ "VALUES	(?		 , ?			 , ?		   , ?			 ,?)", Statement.RETURN_GENERATED_KEYS);
				ps.setString(1, pickUpAgentIdFromRequest);
				...
				ps = conn.prepareStatement("update p_cases set c_pickupagentpmtid=? , c_settled='FULL' where c_id = ?");
	...
	private double getTotaAmtToPay( Connection conn , ArrayList<String>caseList) throws Exception {
		...
		ResultSet rs = null;
		try {
			String sql = "select ifnull( (sum((case when c_shipmentpaidbysender ='Y' then 0 else c_receiptamt end)) - sum((c_shipment_cost)) - sum(c_sendmoney)),0)as totdue "
					+ " from  p_cases where c_id in (";
			boolean first = true;
	...
	private double getTotaAmtToPayForCustomersUnderPickUpAgent( Connection conn , String pickUpAgentId, String fromDate, String toDate) throws Exception {
		double amt = 0.0;
		PreparedStatement ps = null;
		ResultSet rs = null;
		...
		try {
			...
			String querycall = "select (sum( (case when q_stage='cncl' and  c_shipmentpaidbysender ='Y' then 0  else c_receiptamt end ) ) - sum((c_shipment_cost)) - sum(c_sendmoney)) " + 
					" from p_cases " + 
					" join p_queue on (c_id = q_caseid and q_status !='CLS' and ( (q_stage='dlv_stg' and q_step='delivered') or ( q_stage='cncl' and  c_shipmentpaidbysender ='Y' ) )  )" + 
					" where c_pickupagent=?  and c_settled !='FULL' and c_receiptfromsystem='N'  ";
				
			if(dateIsNotNull)
				querycall += "  and  (c_createddt >=? and c_createddt<adddate(date(?),1))  ";
				
			ps = conn.prepareStatement(querycall); // create a statement
			ps.setString(1, pickUpAgentId); 
			if(dateIsNotNull) {
	...
	private ArrayList<String> getCid( Connection conn , String pickUpAgentId, String fromDate, String toDate) throws Exception {
		...
		try {
			...
			String querycall = "select c_id From p_cases  "
				...
				+ " left join kbstate on st_code = c_rcv_state"
				+ " where c_pickupagent =? and c_settled !='FULL' and c_receiptfromsystem='N'  "
				+ " and ( (q_stage='dlv_stg' and q_step='delivered') or (c_shipmentpaidbysender ='Y' and q_stage='cncl'))   ";
