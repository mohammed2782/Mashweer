 ...
 * 
 */
public class CustomerBanlanceInAdvance extends CoreMgr{
		...
		public CustomerBanlanceInAdvance() {
			records = 0;
			MainSql = "select '' as selectedcases,'' as selectedcaseshidden, c_shipmentpaidbysender, q_stage, q_step,'' as status ,'' as pmtCheckBox,  'شحنات سلمت وراجعه فقط' as title, c_custid,c_custreceiptnoori,'' as totamt,'' as pmtrmk, '' as pmtdate, "
					...
					+ " where c_custid ={customerAcct} and c_settled !='FULL' "
					+ " and ( (q_stage='dlv_stg' and q_step='with_agent') or ( q_stage='init') ) "
					+ " and c_paidinadvance='NO' and c_advancepmtid=0 and q_status ='ACTV' and c_receiptfromsystem='N'  order by c_custreceiptnoori ";
			...
			userDefinedNewColsDefualtValues.put("totamt", new String [] {"%select "
					...
					+ " and c_custid='{customerAcct}' and c_paidinadvance='NO' and c_advancepmtid=0 and q_status ='ACTV' "
					+ " and c_settled !='FULL' and c_receiptfromsystem='N'  "});
		...
		public void initialize(HashMap smartyStateMap){
			...
			try {
				...
				pst = conn2.prepareStatement("select c_custreceiptnoori "
					...
					+ " and c_settled !='FULL' and c_receiptfromsystem='N' order by c_custreceiptnoori  ");
				...
				if (checkBoxPayment) {
					...
					userDefinedNewLookups.put("totamt", "!select ifnull( (sum( (c_receiptamt)) - sum(c_shipment_cost) - sum(c_sendmoney) ),0)  "
							...
							+ " and c_custid='"+custId+"'  and c_settled !='FULL' and  (c_createddt >= '{fromdate}' "
									+ " and c_createddt<=adddate(date('{todate}'),1)) and c_receiptfromsystem='N' ");
				}
			}catch(Exception e) {
				e.printStackTrace();
			}finally {
			...
			if (colName.equalsIgnoreCase("c_custreceiptnoori") ) {
				if (!errorFlag) {
					...
					return "<td colspan='2' stye='font-size: 15px;' align='center'><label> المبلغ الصافي للعميل </label>"
							...
							+ " name=\"myClassBean\" value=\"com.app.incomeoutcome.CustomerBanlanceInAdvance\">"
			...
			try {
				...
				if (!cidList.isEmpty()) {//make sure u have cases
					ps = conn.prepareStatement( "INSERT INTO p_inadvance_cust_pmt "
							+ " 		(advpmt_custid, advpmt_amt, advpmt_date, advpmt_createdby,advpmt_rmk)"
					...
					ps = conn.prepareStatement("update p_cases set c_advancepmtid=?, c_priceb4change=c_receiptamt, c_paidinadvance='YES' , c_settled='FULL' where c_id = ?");
