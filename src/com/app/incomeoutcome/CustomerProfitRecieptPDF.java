...
public class CustomerProfitRecieptPDF {
	...
	private  PdfPTable getShipmentsTable(Connection conn ,CustomerPaymentBean cpb,Font font){
        ...
        try{
	        ...
	        relativeWidths [6] = 7; // net
	        relativeWidths [7] = 8; // shipment cost
	        relativeWidths [8] = 12; // receipt amt
	        relativeWidths [9] = 12; // hp
	        relativeWidths [10] = 13; // address
	        relativeWidths [11] = 10; // receipt no 
	        relativeWidths [12] = 5; // seq
        	...
        	for (CaseInformation ci  : shipments){
				...
				//System.out.println("Rural = "+ci.getRural());
				...
				if(ci.getStatus().equalsIgnoreCase("dlv"))
					if (ci.getAdvancedPaymentStatus().equalsIgnoreCase("REFUNDED") && ci.getChangedPrice().equalsIgnoreCase("Y")) {
						ph = new Phrase("-",font);
					}else {
				...
				if(ci.getStatus().equalsIgnoreCase("dlv"))
					if (ci.getAdvancedPaymentStatus().equalsIgnoreCase("REFUNDED") && ci.getChangedPrice().equalsIgnoreCase("Y")) {
						netAmtPerOrder = ci.getReceiptAmt() - ci.getReceiptAmtB4Change();
					}else {
				...
				if(ci.getStatus().equalsIgnoreCase("dlv"))
					if ( ci.getChangedPrice().equalsIgnoreCase("Y")) {
						String s = "تم التسليم مع تغيير سعر الوصل من "+ci.getReceiptAmtB4Change()+", إلى "+ci.getReceiptAmt();
						
						if (ci.getAdvancedPaymentStatus().equalsIgnoreCase("REFUNDED")) {
							s += " وقد تم المحاسبه عليه مقدما ";
						}
