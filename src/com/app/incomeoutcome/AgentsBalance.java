...
public class AgentsBalance extends CoreMgr {
	...
	public AgentsBalance () {
		records = 0;
		MainSql = "select c_company_sender, c_changedprice, c_priceb4change, c_receiptamt as currentreceiptprice, c_rmk, '' nettobecollectedfromagent , '' as pmtCheckBox, '' as selectedcases,'' as selectedcaseshidden, c_shipmentpaidbysender,c_rural, c_shipmentpaidbycustomer, c_name, q_stage, q_step,'' as status , 'شحنات سلمت وراجعه ' as title, c_custid,c_custreceiptnoori,'' as totamt,'' as pmtrmk, '' as pmtdate, "
				...
				+ "       when (q_stage='cncl' and c_shipmentpaidbycustomer='Y') then c_receiptamt else 0 end)  c_receiptamt, "
		...
		userDefinedColLabel.put("c_agentshare", "حصة المندوب  من مبلغ الشحن");
		...
		userModifyTD.put("c_agentshare", "modifyAgentAmt({c_agentshare}, {q_stage}, {q_step},{c_shipmentpaidbycustomer})");
		...
		userModifyTD.put("c_receiptamt", "modifyReceiptAmt({c_receiptamt}, {q_stage}, {q_step},{c_shipment_cost},{c_shipmentpaidbycustomer})");
		...
		userDefinedMinDateSelect.put("pmtdate", "new Date()");
		...
		userDefinedNewColsDefualtValues.put("totamt", new String [] {"%select "
				...
				+ "  or (c_shipmentpaidbycustomer='Y' and q_stage='cncl')   or (c_shipmentpaidbycustomer='Y' and q_stage='cncl')) ) "
	...
	public void initialize(HashMap smartyStateMap){
		...
		String userStorCod = replaceVarsinString(" {userstorecode} ", arrayGlobals).trim();
		...
		try {
			...
			pst = conn2.prepareStatement("select c_custreceiptnoori "
				...
				+ " and (c_branchcode=? or '{superRank}'='Y')"
				+ "  order by c_custreceiptnoori");
			
			pst.setString(1,agentId);
			pst.setString(2, userStorCod);
			rs = pst.executeQuery();
			while (rs.next()) {
			...
			+ " sum("
			+ "    (case when ( (q_stage='dlv_stg' and q_step='delivered') or  (q_stage ='cncl' and c_shipmentpaidbycustomer ='Y'))"
			+ "  then c_receiptamt else 0 end)) as agentcollected "
			+ " from p_cases "
			+ " join p_queue on (c_id = q_caseid and q_status !='CLS')"
			+ " left join kbstate on st_code = c_rcv_state"
			...
			if (checkBoxPayment) {
				...
				userDefinedNewLookups.put("totamt", "!select ifnull( (sum(c_agentshare)),0) from p_cases "
						+ " join p_queue on (c_id = q_caseid and q_status !='CLS' and ( (q_stage='dlv_stg' and q_step='delivered') "  
						+ "  or (c_shipmentpaidbycustomer='Y' and q_stage='cncl')   or (c_shipmentpaidbycustomer='Y' and q_stage='cncl')) ) "  
						+ " where c_assignedagent='"+agentId+"' and  c_receiptfromsystem = 'N'  and c_agentsharesettled !='FULL'"
	...
	public String myFooterFunction(String colName) {
		...
		}else if(colName.equalsIgnoreCase("c_receiptamt") || colName.equalsIgnoreCase("status"))
			return "";
		else
			return "<td></td>";
	}
	...
	private ArrayList<String> getCid(String custid, Connection conn, String fromDate, String toDate) throws Exception {
		...
		try {
			...
			String querycall = "select c_id From p_cases "
				...
				+ " where c_assignedagent =? and c_agentsharesettled !='FULL' and c_receiptfromsystem = 'N' "
				+ " and ( (q_stage='cncl')  or (q_stage='dlv_stg' and q_step='delivered')  "
				+ " or (c_shipmentpaidbycustomer='Y' and q_stage='cncl') or (c_shipmentpaidbysender='Y' and q_stage='cncl') ) "; //any stage of cancel and delivered
			if(dateIsNotNull)
