...
public class customer_payment extends CoreMgr {
	...
	public customer_payment() {
		 ...
		 * operations
		 */
		MainSql = " select cp_id ,cp_custid,cp_amount_paid ,cp_paymentdt , cp_createdby ,cp_rmk, '' as fake, '' as profit "
				+ " from  p_customer_payments where cp_custid = '{customerAcct}' and cp_barcode='N' order by cp_id desc ";
		...
		userModifyTD.put("profit", "printProfiReceipt({cp_id})");
	...
	public String printProfiReceipt(HashMap<String, String> hashy) {
		String btn = "<a href=\"../TLKProfitReceiptSRVL?cp_id="+hashy.get("cp_id")+"\" "
	...
	public String doDelete(HttpServletRequest rqs) {
		...
		try {
			...
			pst = conn.prepareStatement("insert into p_deletedpaymets "
					+ " (dp_pmtid, dp_custid, dp_pmtamt, dp_pmtdate, dp_pmtrmk, dp_pmtcreateddt, dp_pmtcreatedby, dp_deletedby)" + 
					" SELECT cp_id, cp_custid,cp_amount_paid , cp_paymentdt, cp_rmk, cp_createddt, cp_createdby, ?  from p_customer_payments " + 
					" where cp_id = ?");
			pst.setString(1, userId);
			pst.setString(2, keyVal);
			pst.executeUpdate();
