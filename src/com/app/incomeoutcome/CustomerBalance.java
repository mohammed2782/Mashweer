...
public class CustomerBalance extends CoreMgr{
		...
		public CustomerBalance() {
			records = 0;
			MainSql = "select c_priceb4change, c_receiptamt as currentreceiptprice, c_changedprice, c_paidinadvance,c_advancepmtid,  '' as selectedcases,'' as selectedcaseshidden, c_shipmentpaidbysender, q_stage, q_step,'' as status ,'' as pmtCheckBox,  'شحنات سلمت وراجعه فقط' as title, c_custid,c_custreceiptnoori,'' as totamt,'' as pmtrmk, '' as pmtdate, "
					...
					+ " (case when (q_stage='dlv_stg' and q_step='delivered' and c_paidinadvance ='NO') then c_receiptamt"
					+ "       when ( q_stage='dlv_stg' and q_step ='delivered' and c_paidinadvance ='YES' and c_changedprice='Y' ) then (c_receiptamt - c_priceb4change) "
					...
					+ "  c_shipment_cost, "
					+ " (	case "
					+ "			when (q_stage='dlv_stg' and q_step='delivered' and  c_paidinadvance ='NO')  then (c_receiptamt - c_shipment_cost) "
					+ " 		when ( q_stage='dlv_stg' and q_step ='delivered' and c_paidinadvance ='YES' and c_changedprice='Y' ) then c_receiptamt - c_priceb4change  "
					...
					+ "        or (q_stage='dlv_stg' and q_step='delivered' and c_paidinadvance ='YES' and c_changedprice='Y' and ifnull(c_pmtid,0) = 0) "
			...
			userModifyTD.put("c_receiptamt", "modifyReceiptAmt({c_changedprice},{c_receiptamt}, {q_stage}, {q_step}, {c_shipmentpaidbysender},{c_paidinadvance},{c_advancepmtid})");
			userModifyTD.put("c_shipment_cost", "modifyShipmentCost( {c_changedprice},{c_shipment_cost}, {q_stage}, {q_step}, {c_shipmentpaidbysender},{c_paidinadvance},{c_advancepmtid})");
			userModifyTD.put("status", "modifyStatus({c_priceb4change}, {currentreceiptprice}, {c_changedprice}, {q_stage}, {q_step}, {c_shipmentpaidbysender},{c_paidinadvance},{c_advancepmtid})");
		...
		public void initialize(HashMap smartyStateMap){
			...
			try {
				...
				String sql = "select c_custreceiptnoori ,"
						+ "ifnull(sum( (	"
						+ "case when (q_stage='dlv_stg' and q_step='delivered' and c_paidinadvance ='NO')  then (c_receiptamt - c_shipment_cost) "
						+ " when ( q_stage='dlv_stg' and q_step ='delivered' and c_paidinadvance ='YES' and c_changedprice='Y' ) then c_receiptamt - c_priceb4change  "
						+ "	when ( q_stage='cncl' and q_status !='CLS' and c_shipmentpaidbysender='N' and  c_paidinadvance='YES' and c_advancepmtid>0 and c_settled ='FULL') then (c_shipment_cost - c_receiptamt) "
						+ " when ( q_stage='cncl' and q_status !='CLS' and c_shipmentpaidbysender='Y' and  c_paidinadvance='YES' and c_advancepmtid>0 and c_settled ='FULL') then (c_receiptamt*-1) "
		...
		public String doInsert(HttpServletRequest rqs, boolean autoCommit) {
			...
			try {
				...
				if (!cidList.isEmpty()) {//make sure u have cases
					...
					ps = conn.prepareStatement("update p_cases set c_pmtid=? , c_settled='FULL'  , "
							+ "c_paidinadvance=(case when c_paidinadvance='YES' then 'REFUNDED' else 'NO' end)   where c_id = ?  ");
