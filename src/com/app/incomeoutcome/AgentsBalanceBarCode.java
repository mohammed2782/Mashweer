...
public class AgentsBalanceBarCode  extends CoreMgr {
	...
	public AgentsBalanceBarCode () {
		...
		MainSql = "select '' nettobecollectedfromagent , '' as pmtCheckBox, '' as selectedcases,'' as selectedcaseshidden, c_shipmentpaidbysender,c_rural, c_shipmentpaidbycustomer, c_name, q_stage, q_step,'' as status , 'شحنات سلمت وراجعه ' as title, c_custid,c_custreceiptnoori,'' as totamt,'' as pmtrmk, '' as pmtdate, "
				...
				+ " where c_assignedagent ={agentAcctBarCode} and c_agentsharesettled !='FULL' and c_receiptfromsystem = 'Y'  "
		...
		userDefinedNewColsDefualtValues.put("c_assignedagent", new String[] {"{agentAcctBarCode}"});
		userDefinedLookups.put("c_assignedagent", "select us_id, us_name from kbusers where us_id='{agentAcctBarCode}'");
		...
		userDefinedNewColsDefualtValues.put("totamt", new String [] {"%select "
				...
				+ " where c_assignedagent='{agentAcctBarCode}' and  c_receiptfromsystem = 'Y'  and c_agentsharesettled !='FULL' "});
		...
		if (colName.equalsIgnoreCase("c_custreceiptnoori") ) {
			if (!errorFlag) {
				...
				return "<td colspan='3' stye='font-size: 15px;' align='center'><table><tr><td><label>المبلغ الكلي للشحنات </label></td>"
						...
						+ " name=\"myClassBean\" value=\"com.app.incomeoutcome.AgentsBalanceBarCode\">"
	...
	public String doInsert(HttpServletRequest rqs, boolean autoCommit) {
		...
		String agentId = replaceVarsinString(" {agentAcctBarCode} ", arrayGlobals).trim();
		try {
			...
			//get list of cids
			ArrayList<Integer> cidList = getCid(agentId, conn);
			if (!cidList.isEmpty()) {//make sure u have cases
				ps = conn.prepareStatement( "INSERT INTO p_agent_payments "
						+ " 		(ap_agentid, ap_amount_paid, ap_paymentdt, ap_createdby, ap_rmk, ap_barcode)"
	...
	private ArrayList<Integer> getCid(String custid, Connection conn) throws Exception {
		...
		try {
				String querycall = "select c_id From p_cases "
					+ " join p_queue on (c_id= q_caseid and q_status !='CLS')"
					+ " left join kbstate on st_code = c_rcv_state"
					+ " where c_assignedagent =? and c_agentsharesettled !='FULL' and c_receiptfromsystem = 'Y' "
